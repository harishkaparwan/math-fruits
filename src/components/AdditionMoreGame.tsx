...
const AdditionMoreGame: React.FC<TaskComponentProps> = ({ onComplete }) => {
    ...
    if (answer === `${correctAnswer}`) {
      ...
      setTimeout(() => {
        ...
        if (questionCount < totalQuestion-1) {
          ...
          setQuestionCount(totalQuestion); // Ensure count reflects completion
        }
      }, 3000);
    } else {
      setShowWrong(true);
...
useEffect(() => {
    persistAtomToLocalStorage('Addition_questions', student.questions);
    persistAtomToLocalStorage('Addition_studentName', student.name);
  }, [isComplete]);
  const n1 = Math.min(parseInt(num1) || 0, 9);
  const n2 = Math.min(parseInt(num2) || 0, 9);
    ...
    <div>
      <NameContainer>
        <NameInput
          ...
          disabled={questionCount > 0 || isComplete} // Disable after first question or completion
        />
        {isComplete && (
          <FlashingTitle
            animate={{
      ...
      {!isComplete && (
        <TitleContainer>
          <StaticTitle>Addition [Question # {questionCount+1}]:-</StaticTitle>
          <FlashingTitle
            animate={{
              scale: [1, 1.05, 1],
              backgroundColor: ['#fff3e0', '#ffe0b2', '#fff3e0'],
      ...
      <Form>
        <Grid>
          ...
          <div /> {/* Empty cell to push fruits to next row */}
          <FruitDisplay>
            {Array(randomNum1 + randomNum2).fill('🍎').map((fruit, i) => (
              <motion.span
                key={`result-${i}`}
                initial={{ scale: 0 }}
                animate={{ scale: answer === `${randomNum1 + randomNum2}` ? 1 : 0 }}
...
export default AdditionMoreGame;